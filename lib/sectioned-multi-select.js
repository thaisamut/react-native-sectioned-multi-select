...
const noResults = <Text>ไม่พบข้อมูล</Text>
...
class SectionedMultiSelect extends PureComponent {
  ...
  _filterItems = async (searchTerm) => {
    ...
    const params = { "params.query":searchTerm };    
    const results = await OLI.Fetch
      .post(url, params , { showLoading: false })
      .then(result => {
      ...
      .catch((error) => {
        ...
        console.error("ERROR:::_filterItems() => ", error.message, error);
      })
      this.setState({ searchTerm, renderItems: results });
      return results;
  }
    ...
    return (
      <View>
        ...
        <View>
          {item[subKey] &&
          <FlatList
            keyExtractor={i => `key-${i[uniqueKey]}`}
            data={item[subKey]}
  ...
  render = () => {
    ...
    const { searchTerm, selector, renderItems } = this.state
              ...
              <View style={[{ flexDirection: 'row', paddingVertical: 5 }, this.styles.searchBar]}>
                <View style={this.styles.center}>
                  ...
                  <Icon
                    name="search"
                    size={18}
                    style={{ marginHorizontal: 15, color:"#f7982c" }}
                  />}
                </View>
                <TextInput
                  selectionColor={colors.searchSelectionColor}
                  onChangeText={searchTerm => this._filterItems(searchTerm.trim())}
